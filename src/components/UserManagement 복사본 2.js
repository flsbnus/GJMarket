...
const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // FormData 객체 생성 (Spring에서 기본적으로 application/x-www-form-urlencoded 처리를 위해)
  ...
  return (
    <div className="max-w-md mx-auto mt-8 p-6 bg-white rounded-lg shadow-md">
      ...
      {message && (
        <div className={`p-4 rounded mb-4 ${message.includes('완료') ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
          {message}
        </div>
      )}
